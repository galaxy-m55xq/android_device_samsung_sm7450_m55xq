...
enum battery_property_id {
	...
	BATT_POWER_NOW,
	BATT_POWER_AVG,
	/*M55 code for SR-QN6887A-01-531 by gaoxugang at 20230919 start*/
	BATT_ONLINE,
	/*M55 code for SR-QN6887A-01-531 by gaoxugang at 20230919 end*/
	BATT_PROP_MAX,
};
...
/*M55 code for SR-QN6887A-01-532 by gaoxugang at 20230925 start*/
enum {
	USB_STATUS_UNPLUG,
	USB_STATUS_CDP,
...
struct battery_chg_dev {
	...
	bool				boot_mode;
	/*M55 code for SR-QN6887A-01-517 by liufurong at 20230921 end*/
};
...
static const int battery_prop_map[BATT_PROP_MAX] = {
	...
	/*M55 code for SR-QN6887A-01-531 by gaoxugang at 20230919 start*/
	[BATT_ONLINE]       = POWER_SUPPLY_PROP_ONLINE,
	/*M55 code for SR-QN6887A-01-531 by gaoxugang at 20230919 end*/
};
...
/*M55 code for QN6887A-400 by shanxinkai at 20231103 start*/
bool g_charge_bright_screen = false;
EXPORT_SYMBOL(g_charge_bright_screen);
/*M55 code for QN6887A-400 by shanxinkai at 20231103 end*/

/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 start*/
static RAW_NOTIFIER_HEAD(usb_check_chain);

int register_usb_check_notifier(struct notifier_block *nb)
{
	return raw_notifier_chain_register(&usb_check_chain,nb);
}
EXPORT_SYMBOL(register_usb_check_notifier);

int unregister_usb_check_notifier(struct notifier_block *nb)
{
	return raw_notifier_chain_unregister(&usb_check_chain,nb);
}
EXPORT_SYMBOL(unregister_usb_check_notifier);

int usb_check_notifier_call_chain(unsigned long val,void *v)
{
	pr_err("%s: charger state is %d!", __func__, val);
	return raw_notifier_call_chain(&usb_check_chain,val,v);
}
EXPORT_SYMBOL(usb_check_notifier_call_chain);
...
bool get_boot_mode(void) {
    ...
    if (!touchload) {
        pr_err("%s: it is not normal boot!", __func__);
    } else if (strnstr(touchload, "touch.load=1", strlen(touchload))) {
        pr_err("%s: it is normal boot!", __func__);
        return true;
    }
...
/*M55 code for SR-QN6887A-01-532 by gaoxugang at 20230925 start*/
static int usb_status = USB_STATUS_UNPLUG;
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	int rc;
	/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 start*/
	charger_notify_t charger_state = CHARGER_NOTIFY_START_CHARGING;
	static charger_notify_t pre_charger_state = CHARGER_NOTIFY_UNKNOWN;
	/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 end*/
	...
	case POWER_SUPPLY_USB_TYPE_CDP:
		usb_status = USB_STATUS_CDP;
		usb_psy_desc.type = POWER_SUPPLY_TYPE_USB;//POWER_SUPPLY_TYPE_USB_CDP;
		break;
	...
	case POWER_SUPPLY_USB_TYPE_PD_PPS:
		...
		break;
	default:
		usb_status = USB_STATUS_UNPLUG;
		usb_psy_desc.type = POWER_SUPPLY_TYPE_USB;
		/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 start*/
		charger_state = CHARGER_NOTIFY_STOP_CHARGING;
		/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 end*/
		break;
	}
	/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 start*/
	if (pre_charger_state != charger_state) {
		usb_check_notifier_call_chain(charger_state, NULL);
		pre_charger_state = charger_state;
	}
	/*M55 code for SR-QN6887A-01-356 by tangzhen at 20231007 end*/
...
/*M55 code for SR-QN6887A-01-549 by shixuanxuan at 20230831 start*/
static int otg_psy_get_prop(struct power_supply *psy,
		enum power_supply_property prop,
		union power_supply_propval *pval)
{
	pr_err("otg_psy_get_prop enter");
	return 0;
}

static int otg_psy_set_prop(struct power_supply *psy,
		enum power_supply_property prop,
		const union power_supply_propval *pval)
{
	pr_err("otg_psy_set_prop enter");
	return 0;
}
...
static int otg_psy_prop_is_writeable(struct power_supply *psy,
		enum power_supply_property prop)
{
	switch (prop) {
	default:
...
static struct power_supply_desc otg_psy_desc = {
	.name			= "otg",
	.type			= POWER_SUPPLY_TYPE_MAINS,
	.properties		= otg_props,
	.num_properties		= ARRAY_SIZE(otg_props),
	.get_property		= otg_psy_get_prop,
	.set_property		= otg_psy_set_prop,
	.property_is_writeable	= otg_psy_prop_is_writeable,
};
/*M55 code for SR-QN6887A-01-549 by shixuanxuan at 20230831 end*/
/*M55 code for QN6887A-2211 by xiongxiaoliang at 20231202 start*/
#if defined(CONFIG_CUSTOM_FACTORY_BUILD)
const int g_fac_min_fcc_lmt = 2500000;
#endif //CONFIG_CUSTOM_FACTORY_BUILD
/*M55 code for QN6887A-2211 by xiongxiaoliang at 20231202 end*/
					...
					u32 fcc_ua)
{
	...
	/*M55 code for QN6887A-2211 by xiongxiaoliang at 20231202 start*/
	#if defined(CONFIG_CUSTOM_FACTORY_BUILD)
	if (fcc_ua < g_fac_min_fcc_lmt) {
		fcc_ua = g_fac_min_fcc_lmt;
	}
	#endif //CONFIG_CUSTOM_FACTORY_BUILD
	/*M55 code for QN6887A-2211 by xiongxiaoliang at 20231202 end*/
					...
					int val)
{
	...
	/*M55 code for QN6887A-400 by shanxinkai at 20231103 start*/
	if ((g_charge_bright_screen == true) && (val != 0)) {
		val += THERMAL_LEVEL_OFFSET;
	}

	pr_err("the screen bright = %d, thermal_level = %d\n", g_charge_bright_screen, val);
	/*M55 code for QN6887A-400 by shanxinkai at 20231103 end*/
...
/*M55 code for P240108-01840 by xiongxiaoliang at 20240111 start*/
#ifndef CONFIG_CUSTOM_FACTORY_BUILD
typedef enum qti_online_state {
    QTI_OFFLINE = 0,
    QTI_ONLINE_AC,
    QTI_ONLINE_USB,
} qti_online_state_t;

static qti_online_state_t qti_get_online_state(void)
{
    qti_online_state_t ret = QTI_OFFLINE;
    ...
    pr_debug("%s: get usb type val = %d, voltage now = %d\n", __func__, val.intval, val_vol.intval);
    switch (val.intval) {
        case POWER_SUPPLY_USB_TYPE_DCP:
        case POWER_SUPPLY_USB_TYPE_PD_DRP:
        case POWER_SUPPLY_USB_TYPE_PD_PPS:
    ...
    pr_debug("%s: final ret = %d\n", __func__, ret);
    return ret;
}
#endif
/*M55 code for P240108-01840 by xiongxiaoliang at 20240111 end*/
		...
		union power_supply_propval *pval)
{
	...
	/*M55 code for P240108-01840 by xiongxiaoliang at 20240111 start*/
	#ifndef CONFIG_CUSTOM_FACTORY_BUILD
	qti_online_state_t qti_online_state = QTI_OFFLINE;
	#endif
	/*M55 code for P240108-01840 by xiongxiaoliang at 20240111 end*/
	...
	case POWER_SUPPLY_PROP_STATUS:
		pval->intval = pst->prop[prop_id];
		if (!bcdev->boot_mode && pval->intval == POWER_SUPPLY_CHARGING_STATUS_NOT_CHARGING) {
			pval->intval = POWER_SUPPLY_CHARGING_STATUS_CHARGING;
			pr_err("power off charge mode, don't report not charging state");
		}
		/*M55 code for P240108-01840 by xiongxiaoliang at 20240111 start*/
		#ifndef CONFIG_CUSTOM_FACTORY_BUILD
		qti_online_state = qti_get_online_state();
		if ((qti_online_state != QTI_OFFLINE) && (pval->intval == POWER_SUPPLY_CHARGING_STATUS_DISCHARGING)) {
			pval->intval = POWER_SUPPLY_CHARGING_STATUS_NOT_CHARGING;
		}
		#endif
		/*M55 code for P240108-01840 by xiongxiaoliang at 20240111 end*/
		break;
	...
	/*M55 code for SR-QN6887A-01-548|SR-QN6887A-01-516 by xiongxiaoliang at 20230904 start*/
	pr_debug("prop=%d, intval=%d\n", prop, pval->intval);
	/*M55 code for SR-QN6887A-01-548|SR-QN6887A-01-516 by xiongxiaoliang at 20230904 end*/
...
static int battery_chg_init_psy(struct battery_chg_dev *bcdev)
{
	...
	if (IS_ERR(bcdev->psy_list[PSY_TYPE_AC].psy)) {
		...
		bcdev->psy_list[PSY_TYPE_AC].psy = NULL;
		pr_err("Failed to register ac power supply, rc=%d\n", rc);
		return rc;
	}
	/*M55 code for SR-QN6887A-01-590 by liufurong at 20230817 end*/
	/*M55 code for SR-QN6887A-01-549 by shixuanxuan at 20230831 start*/
	bcdev->psy_list[PSY_TYPE_OTG].psy =
		devm_power_supply_register(bcdev->dev, &otg_psy_desc, &psy_cfg);
	if (IS_ERR(bcdev->psy_list[PSY_TYPE_OTG].psy)) {
		rc = PTR_ERR(bcdev->psy_list[PSY_TYPE_OTG].psy);
		bcdev->psy_list[PSY_TYPE_OTG].psy = NULL;
		pr_err("Failed to register otg power supply, rc=%d\n", rc);
		return rc;
	}
	/*M55 code for SR-QN6887A-01-549 by shixuanxuan at 20230831 end*/
