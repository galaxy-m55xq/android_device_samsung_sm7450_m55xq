...
#endif
#if IS_ENABLED(CONFIG_USB_NOTIFY_EXTCON)
	struct device *dev;
	struct extcon_dev *extcon;
#endif
	enum usb_role c_role; 
};
#endif
...
#define MAX_ERROR_RECOVERY_TRIES	3
	...
	/*M55 code for SR-QN6887A-01-552 by wenyaqi at 20230920 start*/
	bool			usb_data_enabled;
	/*M55 code for SR-QN6887A-01-552 by wenyaqi at 20230920 end*/
...
/*M55 code for SR-QN6887A-01-552 by wenyaqi at 20230920 start*/
static enum usb_role s_prev_role = USB_ROLE_NONE;
/*M55 code for SR-QN6887A-01-552 by wenyaqi at 20230920 end*/
static int dwc3_msm_set_role(struct dwc3_msm *mdwc, enum usb_role role)
{
	...
	if (o_notify){
		pdata = (struct usb_notifier_platform_data *)(get_notify_data(o_notify));
		/*M55 code for SR-QN6887A-01-552 by wenyaqi at 20230920 start*/
		s_prev_role = pdata->c_role;
		/*M55 code for SR-QN6887A-01-552 by wenyaqi at 20230920 end*/
...
static struct dwc3_msm *gs_mdwc = NULL;
void usb_notify_control(bool data_enabled)
{
	static bool s_prev_enabled = true;

	if (IS_ERR_OR_NULL(gs_mdwc)) {
		pr_err("%s: didn't get mdwc\n", __func__);
		return;
	}

	gs_mdwc->usb_data_enabled = data_enabled;
	if(s_prev_enabled != gs_mdwc->usb_data_enabled) {
		s_prev_enabled = gs_mdwc->usb_data_enabled;
		pr_err("input usb_data_enabled : %d\n", data_enabled);
		if (!gs_mdwc->usb_data_enabled) {
			dwc3_msm_set_role(gs_mdwc, USB_ROLE_NONE);
		} else {
			dwc3_msm_set_role(gs_mdwc, s_prev_role);
		}
	} else {
		pr_info("same usb_data_enabled : %d, no USB role change\n", data_enabled);
	}
}
		...
		unsigned long event, void *ptr)
{
	...
	/*M55 code for QN6887A-348 by shixuanxuan at 20231025 start*/
	struct dwc3_msm *mdwc = container_of(this, struct dwc3_msm, panic_nb);
	struct dwc3 *dwc;
	const struct debugfs_reg32 *dwc3_regs;
	int size, i;
	...
	dwc = platform_get_drvdata(mdwc->dwc3);
	dwc3_regs = dwc->regset->regs;
	size = dwc->regset->nregs;
	/*M55 code for QN6887A-348 by shixuanxuan at 20231025 end*/
...
static int dwc3_otg_start_peripheral(struct dwc3_msm *mdwc, int on)
{
	...
	dwc = platform_get_drvdata(mdwc->dwc3);
	vdwc = container_of(dwc, struct dwc3_vendor, dwc);
	/*M55 code for QN6887A-348 by shixuanxuan at 20231025 end*/
