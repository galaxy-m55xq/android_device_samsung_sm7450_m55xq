...
/*M55 code for P240109-02015 by shixuanxuan at 20240202 start*/
static int ucsi_port_type_set(struct typec_port *port, enum typec_port_type type)
{
	struct ucsi_connector *con = typec_get_drvdata(port);
	int ret = 0;
	...
	if (type == TYPEC_PORT_SRC) {
		ucsi_dr_swap(port, TYPEC_HOST);
		ucsi_pr_swap(port, TYPEC_SOURCE);
	} else if (type == TYPEC_PORT_SNK) {
		ucsi_dr_swap(port, TYPEC_DEVICE);
		ucsi_pr_swap(port, TYPEC_SINK);
	} else {
		ret = usb_role_switch_set_role(con->usb_role_sw, USB_ROLE_NONE);
		msleep(2000);
		ret = usb_role_switch_set_role(con->usb_role_sw, USB_ROLE_DEVICE);
		if (ret) {
			dev_err(con->ucsi->dev, "con:%d: failed to set usb role\n", con->num);
		}
		ucsi_dr_swap(port, TYPEC_DEVICE);
		ucsi_pr_swap(port, TYPEC_SINK);
	}
...
/*M55 code for P240109-02015 by shixuanxuan at 20240202 end*/

static const struct typec_operations ucsi_ops = {
	.dr_set = ucsi_dr_swap,
	.pr_set = ucsi_pr_swap,
	/*M55 code for P240109-02015 by shixuanxuan at 20240202 start*/
	.port_type_set = ucsi_port_type_set
	/*M55 code for P240109-02015 by shixuanxuan at 20240202 end*/
};
