...
static bool is_zeroed_page(struct page *page, unsigned int start, size_t len)
{
	return !memchr_inv(page_address(page) + start, 0, len);
}
...
out_err:
		if (PageUptodate(page)) {
			...
			if (!is_zeroed_page(page, 0, F2FS_BLKSIZE))
