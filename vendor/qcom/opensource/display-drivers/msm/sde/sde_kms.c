		...
		struct drm_atomic_state *old_state)
{
	...
	struct sde_vm_ops *vm_ops;
	/*M55 code for P231229-04010 by hehaoran at 20240123 start*/
	int i = 0, rc = 0, add_count = 0, remove_count = 0, crtc_active_cnt = 0;
	/*M55 code for P231229-04010 by hehaoran at 20240123 end*/
	...
	pm_runtime_put_sync(sde_kms->dev->dev);
	/*M55 code for P231229-04010 by hehaoran at 20240123 start*/
	drm_for_each_crtc(crtc, sde_kms->dev) {
		if (crtc->state && crtc->state->active) {
			crtc_active_cnt++;
		}
	}
	...
	for_each_old_crtc_in_state(old_state, crtc, old_crtc_state, i) {
		...
		if (crtc->state->active_changed && !crtc->state->active && crtc_active_cnt == 0) {
			remove_count = atomic_read(&sde_kms->dev->dev->power.usage_count);
			pr_info("[start] %s:Usage count:%d\n",__func__,
...
static int _sde_kms_drm_obj_init(struct sde_kms *sde_kms)
{
	...
	for (i = 0; i < priv->num_encoders; i++) {
		...
		if (catalog->max_cwb > 0)
			priv->encoders[i]->possible_clones = sde_encoder_get_clones(priv->encoders[i]);
	}
